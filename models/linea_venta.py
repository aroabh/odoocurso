...
class LineaVenta(models.Model):
    _name = 'reto.linea.venta'
    _description = 'Detalle de Venta'

    name = fields.Char(string='Linea Venta', related='venta_id.name')
    ...
    prenda_id = fields.Many2one(
        comodel_name='reto.prenda', inverse_name='linea_venta_ids', string='Prenda',
    ...
    precio_p_venta = fields.Float(string='Precio prenda venta', compute='_calcular_p_venta')
    ...
    def _onchange_precio(self):
        self.p_total = self.cantidad_p * self.precio_p_venta

    @api.depends('cantidad_p', 'precio_p_may')
    def _calcular_p_total(self):
        for rec in self:
            rec.p_total = rec.cantidad_p * rec.precio_p_venta

    @api.depends('precio_p_may')
    def _calcular_p_venta(self):
        for rec in self:
            rec.precio_p_venta = rec.precio_p_may * 1.15